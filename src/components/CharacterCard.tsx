...
export const CharacterCard = memo(function CharacterCard({ build, slotLimit, selected, favorite, comparisonDisabled, onOpen, onCompare, onFavorite, mode }: Props) {
  ...
  return <article className={`character-card character-card--${mode}`}>
    ...
    <div className="character-card__body">
      ...
      <div className="card-footer"><span className="difficulty">{variant.bloodlineSlotCount} Bloodlines · {variant.elementSlotCount} elements</span><button className="button button--text" onClick={() => onOpen(build)}>View build <ArrowUpRight size={15} /></button></div>
    </div>
  </article>
})
