...
export function BuildDetail({ build, onClose }: { build: CharacterBuild; onClose: () => void }) {
  ...
  return <div className="modal-layer" role="dialog" aria-modal="true" aria-label={`${build.name} build details`}><button className="modal-backdrop" onClick={onClose} aria-label="Close details" /><article ref={panelRef} className="detail-panel">
    ...
    <div className="detail-content">
      ...
      <section><Heading n="10" title="Verification" /><p><strong>{variant.verificationStatus}</strong> · {build.confidence} · Last checked for {variant.lastVerifiedUpdate}.</p><p>Known compromises: {(variant.compromises?.length ? variant.compromises : build.knownCompromises).join(' ')}</p>{variant.ownershipRequirements?.length ? <p><strong>Required items:</strong> {variant.ownershipRequirements.join(' · ')}</p> : null}<ul>{build.evidence.map((item) => <li key={`${item.category}-${item.claim}`}><strong>{item.category}:</strong> {item.claim} — {item.notes}</li>)}</ul></section>
    </div>
  </article></div>
}
