...
const emptyFilters = { series: '', bloodline: '', favorites: '', owned: '' }
...
export default function App() {
  ...
  const [customBackupCount, setCustomBackupCount] = useState(0)

  useEffect(() => {
    setCustomBackupCount(migratePublicData().customBuildCount)
  ...
  const filtered = useMemo(() => {
    ...
    }).sort((a, b) => (b.ratings.accuracy * (1 - meta) + b.ratings.pvp * meta) - (a.ratings.accuracy * (1 - meta) + a.ratings.pvp * meta))
  ...
  return <div className="app-shell">
    ...
    {customBackupCount > 0 && <div className="migration-notice" role="status"><span>{customBackupCount} local custom build(s) were removed from the public archive and preserved in a backup.</span><button className="button button--outline" onClick={() => { const data = localStorage.getItem('shindo-build-archive:removed-custom-backup:v3') ?? '[]'; const url = URL.createObjectURL(new Blob([data], { type: 'application/json' })); const anchor = document.createElement('a'); anchor.href = url; anchor.download = 'shindo-personal-build-backup.json'; anchor.click(); URL.revokeObjectURL(url) }}>Download backup</button></div>}
    ...
    {loading ? <main className="loading-page">Loading reviewed builds…</main>
      ...
      : <main>
        <section className="archive-hero"><div className="archive-hero__copy"><span className="eyebrow">CURATED SHINDO LIFE BUILDS</span><h1>Build the fighter.<br /><i>Know the compromise.</i></h1><p>Reviewed manhwa-inspired setups with prepared variants, exact hotbars, and honest verification labels.</p></div><div className="archive-stats"><div><strong>{builds.length}</strong><span>Reviewed builds</span></div><div><strong>{builds.filter((build) => build.confidence === 'Verified').length}</strong><span>Fully verified</span></div><div><strong>{prefs.favorites.length}</strong><span>Favorites</span></div></div></section>
        ...
        <section className="filter-row"><span><SlidersHorizontal size={15} /> Filters</span><select aria-label="Filter by series" value={filters.series} onChange={(e) => setFilters({ ...filters, series: e.target.value })}><option value="">All series</option>{values.series.map((value) => <option key={value}>{value}</option>)}</select><select aria-label="Filter by Bloodline" value={filters.bloodline} onChange={(e) => setFilters({ ...filters, bloodline: e.target.value })}><option value="">All Bloodlines</option>{values.bloodlines.map((value) => <option key={value}>{value}</option>)}</select><select aria-label="Filter by inventory readiness" value={filters.owned} onChange={(e) => setFilters({ ...filters, owned: e.target.value })}><option value="">Any inventory</option><option value="makeable">Builds I can make</option><option value="missing-one">Missing one item</option></select><button className={`favorites-filter ${filters.favorites ? 'active' : ''}`} onClick={() => setFilters({ ...filters, favorites: filters.favorites ? '' : 'only' })}><Heart size={13} /> Favorites</button></section>
        <div className="results-bar"><div><strong>{filtered.length}</strong> REVIEWED RESULTS</div><div className="view-switch"><button className={view === 'builds' && cardMode === 'compact' ? 'active' : ''} onClick={() => { navigate('builds'); setCardMode('compact') }} aria-label="Compact cards"><Grid2X2 size={16} /></button><button className={view === 'builds' && cardMode === 'visual' ? 'active' : ''} onClick={() => { navigate('builds'); setCardMode('visual') }} aria-label="Visual cards"><Rows3 size={16} /></button><button className={view === 'database' ? 'active' : ''} onClick={() => navigate('database')} aria-label="Table view"><Table2 size={16} /></button><button onClick={() => filtered.length && setSelected(filtered[Math.floor(Math.random() * filtered.length)])} aria-label="Random build"><Dice5 size={16} /></button></div></div>
