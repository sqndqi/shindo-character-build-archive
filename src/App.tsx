...
const emptyFilters = { media: '', franchise: '', series: '', status: '', bloodline: '', favorites: '', owned: '' }
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    {loading ? <main className="loading-page">Loading archive…</main>
      ...
      : <main>
        <section className="archive-hero"><div className="archive-hero__copy"><span className="eyebrow">MANHWA · MANGA · ANIME SHINDO LIFE ARCHIVE</span><h1>Build the fighter.<br /><i>Know the compromise.</i></h1><p>Reviewed setups and clearly labeled early drafts across Lookism, Solo Leveling, Naruto, Bleach, One Piece, JoJo, and more.</p></div><div className="archive-stats"><div><strong>{builds.length}</strong><span>Total characters</span></div><div><strong>{builds.filter((build) => build.publicationStatus === 'Reviewed').length}</strong><span>Reviewed builds</span></div><div><strong>{builds.filter((build) => build.publicationStatus === 'Draft').length}</strong><span>Draft builds</span></div><div><strong>{values.series.length}</strong><span>Series represented</span></div></div></section>
